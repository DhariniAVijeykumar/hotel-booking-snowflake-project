...
ORDER BY date;
-- For monthly revenue, in chart click the date drop down in bucketing select month

--Line Chart - Monthly Bookings
SELECT date, total_booking
FROM GOLD_AGG_DAILY_BOOKING
ORDER BY date;
-- For monthly booking, in chart click date drop down, in bucketing select month
